...
def return_rand_cands(embedding_matrix,
                      num_candidates=1):
    """Returns the top candidate replacements."""
    weights = torch.ones(embedding_matrix.shape[0])
    top_k_ids = torch.multinomial(weights, num_candidates)
...
def run_model(args):
    ...
    elif args.saved_label_map is not None:
    ...
    for i in range(args.iters):
        ...
        # Accumulate
        if not args.randomly_sample_cands:
            for step in pbar:
        ...
        if args.randomly_sample_cands:
            candidates = return_rand_cands(embeddings.weight,
                                           num_candidates=args.num_cand).to(device=device)
        else:
    ...
    for trg_domain in best_test_metric:
        logger.info(f'Best test metric {trg_domain}: {best_test_metric[trg_domain]}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--perturbed', action='store_true',
                        help='Perturbed sentence evaluation of relation extraction: replace each object in dataset with '
                             'a random other object')
    parser.add_argument('--patience', type=int, default=5)
    parser.add_argument('--num-cand', type=int, default=10)
    parser.add_argument('--sentence-size', type=int, default=50)
    parser.add_argument('--randomly_sample_cands', action='store_true',
                        help='whether to selcet trigger cands randomly or follow autoprompt algorithm.')
