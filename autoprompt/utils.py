...
class TriggerTemplatizer:
    ...
    def __call__(self, format_kwargs):
        ...
        if not any(trigger_mask[0]) or not any(predict_mask[0]):
            new_sentence = self._tokenizer.decode(input_ids[0][:100], skip_special_tokens=True, clean_up_tokenization_spaces=True)
            format_kwargs['sentence'] = new_sentence
            text = self._template.format(**format_kwargs)
            model_inputs = self._tokenizer.encode_plus(
                text,
                truncation=True,
            ...
            predict_mask = input_ids.eq(self._tokenizer.predict_token_id)
            if not any(trigger_mask[0]) or not any(predict_mask[0]):
                raise Exception(f'Too long example: {text}')
